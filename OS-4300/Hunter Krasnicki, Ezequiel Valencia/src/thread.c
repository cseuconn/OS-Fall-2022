...
thread_create(const char *name)
{
  ...
  //Init thread priority to 1 (medium)
  thread->priority = 1;
...
mi_switch(threadstate_t nextstate)
{
    ...
    /*
     *If our thread did not block or get put to sleep, it used up its time slice and yielded
     *therefore we deceremnt it's priority
     */
    if(cur->priority > 0){
      cur->priority--;
    }
    ...
    /*
     *If our thread was put to sleep, it's likely I/O bound, or waiting on mutexes.
     *therefore we increase its priority
     */
    if(cur->priority < 2){
      cur->priority++;
    }
...
thread_exit(void)
{
  ...
  //Set priority to NULL to indicate that our process is not intended to be run
